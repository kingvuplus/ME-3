...
class ScanSetup(ConfigListScreen, Screen, CableTransponderSearchSupport):
    ...
    def newConfig(self):
        ...
        print 'cur is', cur
        if cur == self.typeOfScanEntry or cur == self.typeOfInputEntry or cur == self.tunerEntry or cur == self.systemEntry or cur == self.preDefSatList or self.modulationEntry and self.systemEntry[1].value == eDVBFrontendParametersSatellite.System_DVB_S2 and cur == self.modulationEntry:
            self.createSetup()
...
#        (eDVBFrontendParametersTerrestrial.FEC_6_7, '6/7'),
    ...
    def keyGoCheckTimeshiftCallback(self, answer):
        ...
        if nim.isCompatible('DVB-S'):
            ...
            if self.scan_typecable.value == 'single_transponder':
                self.addCabTransponder(tlist, self.scan_cab.frequency.value, self.scan_cab.symbolrate.value, self.scan_cab.modulation.value, self.scan_cab.fec.value, self.scan_cab.inversion.value)
                removeAll = False
